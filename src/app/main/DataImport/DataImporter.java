...
public class DataImporter {
     ...
     * aa, ab, ace, ady, af, ak, als, am, ang, an, arc, ar, ary, arz, ast, as, atj,
     * avk, av, awa, ay, azb, az, ban, bar,
     * bat_smg, ba, bcl, be, bg, bh, bi, bjn, bm, bn, bo, bpy, br, bs, bug, bxr, ca,
     * cbk_zam, cdo, ceb, ce, cho,
     * chr, ch, chy, ckb, co, commons, crh, cr, csb, cs, cu, cv, cy, da, de, din,
     * diq, dsb, dty, dv, dz, ee, el, eml, en,
     * eo, es, et, eu, ext, fa, ff, fiu_vro, fi, fj, fo, frp, frr, fr, fur, fy, gag,
     * gan, ga, gcr, gd, glk, gl, gn, gom,
     * gor, got, gu, gv, hak, ha, haw, he, hif, hi, ho, hr, hsb, ht, hu, hy, hyw,
     * hz, ia, id, ie, ig, ik, ilo, incubator,
     * inh, io, is, it, iu, jam, ja, jbo, jv, kaa, kab, ka, kbd, kbp, kg, ki, kj,
     * kk, kl, km, kn, koi, ko, krc, kr, ksh,
     * ks, ku, kv, kw, ky, lad, la, lbe, lb, lez, lfn, lg, lij, li, lld, lmo, ln,
     * lo, lrc, ltg, lt, lv, mad, mai, map_bms,
     * mdf, meta, mg, mhr, min, mi, mk, ml, mn, mnw, mrj, mr, ms, mt, mus, mwl, myv,
     * my, mzn, nah, nap, na,
     * nds_nl, nds, ne, ng, nl, nn, nov, no, nqo, nrm, nso, nv, ny, oc, olo, om, or,
     * os, pag, pam, pap, pa, pcd, pdc,
     * pfl, pih, pi, pl, pms, pnb, pnt, ps, pt, qu, rm, rmy, rn, roa_rup, roa_tara,
     * ro, rue, ru, rw, sah, sat, sa, scn,
     * sco, sc, sd, se, sg, shn, sh, simple, si, skr, sk, sl, smn, sm, sn, sources,
     * so, species, sq, srn, sr, ss, stq,
     * st, su, sv, sw, szl, szy, ta, tcy, tet, te, tg, th, ti, tk, tl, tn, to, tpi,
     * tr, ts, tt, tum, tw, tyv, ty, udm,
     * ug, uk, ur, uz, vec, vep, ve, vi, vls, vo, war, wa, wo, wuu, xal, xh, xmf,
     * yi, yo, za, zea, zh, zu
     * }
     */
    public enum Domain {
    ...
    public Map importTop(Domain domain, LocalDate date, Boolean monthly) throws Exception { // domena, data dla topki i
                                                                                            // flaga czy topka
                                                                                            // miesieczna
    ...
    public Map importNames(Domain domain, String article) throws Exception { // domena i nazwa artykulu w tej domenie
        String jsonStr = importData("https://www.wikidata.org/w/api.php?action=wbgetentities&sites=" + domain +
                "wiki&titles=" + URLEncoder.encode(article, java.nio.charset.StandardCharsets.UTF_8.toString())
    ...
    public List<Integer> importViews(Domain domain, String article, LocalDate date, LocalDate date2) throws Exception { // domena,
                                                                                                                        // nazwa
                                                                                                                        // artykulu
                                                                                                                        // w
                                                                                                                        // domenie
        ...
        String jsonStr = importData("https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/" + domain +
                ".wikipedia.org/all-access/all-agents/"
                + URLEncoder.encode(article, java.nio.charset.StandardCharsets.UTF_8.toString()) + "/daily/" + dateStr
                + "/" + dateStr2);
        if (jsonStr == null) {
    ...
    public Map importViewsByDomainDays(Domain domain, String article, LocalDate date, LocalDate date2)
            throws Exception { // domena, nazwa artykulu w domenie i zakres dat wyswietlen
        if (date.isAfter(date2))
            throw new Exception("Niepoprawny zakres dat.");
        ...
        for (Object x : names.entrySet()) {
            ...
            viewsByDomain.put((String) y.getKey(),
                    importViews(Domain.valueOf((String) y.getKey()), (String) y.getValue(), date, date2));
        }
        return viewsByDomain; // Mapa przyporzadkowuje domenom listy wyswietlen artykulu.
    }

    // Zwraca sumaryczne wyswietlenia artykulu we wszystkich domenach, w zakresie
    // dat.
    public Map importViewsByDomain(Domain domain, String article, LocalDate date, LocalDate date2) throws Exception { // domena,
                                                                                                                      // nazwa
                                                                                                                      // artykulu
                                                                                                                      // w
                                                                                                                      // domenie
    ...
    public static void main(String[] args) {
        ...
        /*
         * try {
         * Map views = test.importViewsByDomain(Domain.pl, "Polska", LocalDate.of(2020,
         * 12, 12), LocalDate.of(2020, 12, 20));
         * //Map names = test.importNames(Domain.en, "Star_Wars");
         * for (Object x : views.entrySet()) {
         * Map.Entry y = (Map.Entry) x;
         * System.out.println(y.getKey() + " : " + y.getValue());
         * }
         * Map views2 = test.importViewsByDomainDays(Domain.pl, "Polska",
         * LocalDate.of(2020, 12, 12), LocalDate.of(2020, 12, 20));
         ...
         * for (Object x : names.entrySet()) {
         * Map.Entry y = (Map.Entry) x;
         * System.out.println(test.getLink(Domain.valueOf((String)y.getKey()), (String)
         * y.getValue()));
         * }
         */

        /*
         * int views = test.importViews(Domain.pl, "Gwiezdne_wojny", LocalDate.of(2020,
         * 12, 12));
         * System.out.println(views);
         */

        /*
         * List x = test.importViews(Domain.en, "Main_Page", LocalDate.of(2020,12,12),
         * LocalDate.of(2020, 12, 15));
         * System.out.println(x);
         */
        try {
            System.out.println(
                    test.importData("https://www.wikidata.org/w/api.php?action=wbgetentities&sites=plwiki&titles=" +
                            URLEncoder.encode("Świdnica", java.nio.charset.StandardCharsets.UTF_8.toString())
                            + "&languages=en&format=json"));
        } catch (Exception e) {
